	...
	: NodeControlBase( pNode ), mFrameBufferId( 0 )
{
	...
	FUGID( PIN_INPUT_SHADER, "1b5e9ce8-acb9-478d-b84b-9288ab3c42f5" );
	FUGID( PIN_INPUT_GEOMETRY, "6272595e-bb59-407c-9a80-64dc8c99ba22" );
	...
	FUGID( PIN_OUTPUT_RENDER, "249f2932-f483-422f-b811-ab679f006381" );
	FUGID( PIN_OUTPUT_OUTPUT1, "261cc653-d7fa-4c34-a08b-3603e8ae71d5" );

	pinInput( "Trigger", PID_FUGIO_NODE_TRIGGER );

	mPinShader = pinInput( "Shader", PIN_INPUT_SHADER );
	...
	QSharedPointer<fugio::PinInterface>	PinGeometry = pinInput( "Render", PIN_INPUT_GEOMETRY );

	mPinShader->registerPinInputType( PID_RENDER );

	mOutputGeometry = pinOutput<fugio::RenderInterface *>( "Render", mPinOutputGeometry, PID_RENDER, PIN_OUTPUT_RENDER );

	pinOutput( "Output1", PIN_OUTPUT_OUTPUT1 );
...
void ShaderInstanceNode::bindOutputBuffers( QVector<GLenum> &Buffers, QList< QSharedPointer<fugio::PinInterface> > &OutPinLst, int &W, int &H, int &D )
{
	...
	auto GL32 = QOpenGLVersionFunctionsFactory::get<QOpenGLFunctions_3_2_Core>( QOpenGLContext::currentContext() );
...
void ShaderInstanceNode::render( qint64 pTimeStamp, QUuid pSourcePinId )
{
	...
	OpenGLStateInterface		*CurrentState = Q_NULLPTR;
			...
			if( ArrInt )
			{
				...
				switch( UniformData.mType )
				{
					case GL_FLOAT:
						...
						{
							glUniform1fv( UniformData.mLocation, CpyCnt, static_cast<const float *>( ArrDat ) );
						}
						break;
					...
					case GL_FLOAT_VEC2:
						...
						{
							glUniform2fv( UniformData.mLocation, CpyCnt, static_cast<const float *>( ArrDat ) );
						}
					...
					case GL_FLOAT_VEC3:
						...
						{
							glUniform3fv( UniformData.mLocation, CpyCnt, static_cast<const float *>( ArrDat ) );
						}
					...
					case GL_FLOAT_VEC4:
						...
						{
							glUniform4fv( UniformData.mLocation, CpyCnt, static_cast<const float *>( ArrDat ) );
						}
					...
					case GL_INT_VEC2:
						...
						{
							glUniform2iv( UniformData.mLocation, CpyCnt, static_cast<const int *>( ArrDat ) );
						}
					...
					case GL_INT_VEC3:
						...
						{
							glUniform3iv( UniformData.mLocation, CpyCnt, static_cast<const int *>( ArrDat ) );
						}
						break;
					...
					case GL_INT_VEC4:
						...
						{
							glUniform4iv( UniformData.mLocation, CpyCnt, static_cast<const int *>( ArrDat ) );
						}
						break;

					case GL_FLOAT_MAT4:
						if( ArrInt->type() == QMetaType::QMatrix4x4 && ArrInt->elementCount() == 1 && ArrInt->stride() == 16 * sizeof( float ) )
						{
							glUniformMatrix4fv( UniformData.mLocation, CpyCnt, GL_FALSE, static_cast<const float *>( ArrDat ) );
						}
						break;
				}
			}
