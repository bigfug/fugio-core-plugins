...
void OpenGLPlugin::parseShaderErrors( QString pErrorText, QList<SyntaxError> &pErrorData )
{
	...
	for( QString S : SL )
	{
		...
		QRegularExpression	P1( "^ERROR: (.+):(\\d+):\\s+(.+)" );

		if( P1.match( S ).hasMatch() )
		{
			SE.mLineStart = SE.mLineEnd = P1.match( S ).captured( 2 ).toInt() + 1;

			SE.mError = P1.match( S ).captured( 3 );
		...
		QRegularExpression	P2( "^ERROR:\\s?(.+)" );

		if( P2.match( S ).hasMatch() )
		{
			SE.mLineStart = SE.mLineEnd = -1;

			SE.mError = P2.match( S ).captured( 1 );
		...
		QRegularExpression	P3( "^.+\\((\\d+)\\)\\s?:\\s?(.+):\\s?(.+)" );

		if( P3.match( S ).hasMatch() )
		{
			SE.mLineStart   = SE.mLineEnd = P3.match( S ).captured( 1 ).toInt();

			SE.mError = P3.match( S ).captured( 3 );
...
PluginInterface::InitResult OpenGLPlugin::initialise( fugio::GlobalInterface *pApp, bool pLastChance )
{
	...
	connect( mApp->qobject(), SIGNAL(frameEnd(qint64)), this, SLOT(globalFrameEnd(qint64)) );
...
{
	connect( pContext->qobject(), SIGNAL(frameInitialise(qint64)), this, SLOT(contextFrameInitialise(qint64)) );
	connect( pContext->qobject(), SIGNAL(frameEnd(qint64)), this, SLOT(contextFrameEnd(qint64)) );
}

void OpenGLPlugin::contextRemoved(QSharedPointer<ContextInterface> pContext)
{
	disconnect( pContext->qobject(), SIGNAL(frameInitialise(qint64)), this, SLOT(contextFrameInitialise(qint64)) );
	disconnect( pContext->qobject(), SIGNAL(frameEnd(qint64)), this, SLOT(contextFrameEnd(qint64)) );
}

void OpenGLPlugin::contextFrameInitialise( qint64 pTimeStamp )
{
...
void OpenGLPlugin::contextFrameEnd( qint64 pTimeStamp )
{
	QOpenGLContext		*CurCtx = QOpenGLContext::currentContext();
...
void OpenGLPlugin::globalFrameEnd( qint64 pTimeStamp )
{
